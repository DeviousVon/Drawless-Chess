...
class GameOptionsInstrumentedTest {
    ...
    @Test
    fun volumeSliderPreviewsOnlyAfterASelectionSettles() {
        var previewedVolume = -1
        compose.activity.setContent {
            OptionsScreen(
                preferences = GamePreferences(soundVolumePercent = 50),
                onPreferencesChanged = {},
                onSoundVolumePreview = { previewedVolume = it },
                onBack = {},
            )
        }

        compose.onNodeWithTag("option_sound_volume_slider")
            .performSemanticsAction(SemanticsActions.SetProgress) { it(100f) }
        compose.waitUntil(timeoutMillis = 2_000L) { previewedVolume == 100 }

        assertEquals(100, previewedVolume)
    }
    ...
    fun optionsNavigateAndPersistAcrossActivityRecreation() {
        ...
        try {
            ...
            compose.onNodeWithTag("option_haptic_feedback").performScrollTo()
            ...
            compose.onNodeWithTag("options_version")
                .performScrollTo()
                .assertTextEquals(
            ...
            compose.onNodeWithTag("option_haptic_feedback").performScrollTo().assertIsOff()
