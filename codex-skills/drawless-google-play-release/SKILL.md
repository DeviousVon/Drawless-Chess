...
6. Prefer the Google Play Developer Publishing API when an already-authorized external service
...
7. Inspect the live Console before choosing a track. Update the existing Alpha closed-test track
   ...
   from memory.
8. Create or resume an idempotent draft for the exact version code. Upload the AAB path from the
...
9. Fill every locale from the manifest, using Play's locale mapping exactly. Save, move to the
...
10. Confirm the final live state independently on the track page: version code/name, track,
...
Chrome's ChatGPT browser extension must have **Allow access to file URLs** enabled so Codex can
set the AAB file chooser. This is a persistent machine setting, not a per-release task. Check it
before starting release work. If it is disabled, make the one-time setting the only requested
human action; preserve the Play draft and resume immediately afterward. Once enabled, never ask
Bob to select the AAB manually.
