...
2. Before opening or editing Play Console, run the read-only Chrome upload preflight:
   ...
   Require `extensionEnabled: true` and `ready: true`. Confirm the returned Chrome profile is the
   same profile used by the claimed Play Console tab; pass
   ...
   update can clear Chrome's file-URL opt-in even though earlier uploads succeeded. Chrome does not
   expose that toggle reliably in its preferences file, so do not infer its state from the script.
   Verify it authoritatively with the first file-chooser upload attempt before entering release
   notes or making other draft changes.
...
use the preflight above instead of assuming a previous upload proves the current setting. If the
file chooser returns `Not allowed`, ask Bob to open
`chrome://extensions/?id=hehggadaopoacecdllhhajmbjkdcmajg` in that
profile, open **Details**, and enable **Allow access to file URLs**. This protected browser setting
is the only requested human action; never change it through automation. Retry the same verified
file path and resume immediately. Once enabled, never ask Bob to select the AAB manually.
