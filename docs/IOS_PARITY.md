...
| Hints and assistance accounting | coordinator/scoring sources | Complete | Exact hint/scoring paths use the production Apple engine; penalties, counts, threat assistance and score breakdown are exposed and covered by shared tests. |
...
| Checkpoint model and revision semantics | coordinator sources | Complete | Exact checkpoint codec/revision contract is shared and tested; the Swift adapter writes the same JSON payload only when its revision changes. |
...
| Review/analysis | engine review controller and presentation | Partial | Full-strength serial player-move analysis, RC1 grades, progress, summary, mistakes and details UI are implemented; a shared test uses the same serialized native engine session. End-to-end UI evidence remains. |
